...
class FakturpelunasanController extends Controller
{
    public function index()
    {
        ...
        // $fakturs = Faktur_ekspedisi::whereIn('status', ['posting', 'selesai'])
        //     ->whereNull('status_pelunasan')
        //     ->get();
        $fakturs = Faktur_ekspedisi::get();
        $invoices = Tagihan_ekspedisi::get();
        $returns = Faktur_penjualanreturn::get();
        $potonganlains = Potongan_penjualan::get();

        return view('admin.faktur_pelunasan.index', compact('fakturs', 'pelanggans', 'invoices', 'returns', 'potonganlains'));
    }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($data_pembelians as $data_pesanan) {
            ...
            Faktur_ekspedisi::where('id', $detailPelunasan->faktur_ekspedisi_id)->update(['status' => 'selesai', 'status_pelunasan' => 'aktif']);
        }
