...
class FakturekspedisiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('keterangan_tambahan') || $request->has('nominal_tambahan') || $request->has('satuan_tambahan')) {
            for ($i = 0; $i < count($request->keterangan_tambahan); $i++) {
                // Check if either 'keterangan_tambahan' or 'nominal_tambahan' has input
                if (empty($request->keterangan_tambahan[$i]) && empty($request->nominal_tambahan[$i]) && empty($request->satuan_tambahan[$i])) {
                    continue; // Skip validation if both are empty
                }
                ...
                $data_pembelians4->push(['keterangan_tambahan' => $keterangan_tambahan, 'nominal_tambahan' =>$nominal_tambahan, 'satuan_tambahan' => $satuan_tambahan]);
            }
        }
