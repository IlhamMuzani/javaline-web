...
class RekapGajimExport implements FromCollection, WithHeadings, WithMapping, WithTitle
{
    ...
    public function map($detail): array
    {
        return [
            ...
            '',
            'P',
            str_replace('-', '', $this->perhitungan_gaji->tanggal_awal),
            '1390088880046',
            $this->perhitungan_gaji->detail_gajikaryawan->count(),
            $this->perhitungan_gaji->grand_total,
            '',
            '',
            $detail->karyawan->norek ?? '',
            $detail->karyawan->nama_lengkap ?? '',
            $detail->karyawan->alamat3 ?? '',
            $detail->karyawan->alamat2 ?? '',
            $detail->karyawan->alamat ?? '',
            'IDR',
            $detail->gaji_bersih,
            'Trf InHouse',
            '',
            ...
            '',
            '',
            'EPD' . ($this->collection()->search($detail) + 1),
            '',
            '//',
            'info2',
            '//',
