...
@section('content')
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody id="tabel-pembelian">
                                <tr id="pembelian-0">
                                    ...
                                    <td>
                                        <div class="form-group">
                                            <input style="font-size:14px" type="number" class="form-control harga_tarif"
                            ...
                            <tbody>
                                @foreach ($memos as $memo)
                                    <tr data-id="{{ $memo->id }}" data-kode_memo="{{ $memo->kode_memo }}"
                                        data-nama_driver="@if ($memo->kategori == 'Memo Tambahan') {{ $memo->memotambahan->memo->nama_driver }}@else{{ $memo->nama_driver }} @endif"
                                        data-telp_driver="{{ $memo->telp }}"
                                        data-nama_rute="@if ($memo->kategori == 'Memo Tambahan') {{ $memo->memotambahan->memo->nama_rute }}@else{{ $memo->nama_rute }} @endif"
                                        data-kendaraan_id="@if ($memo->kategori == 'Memo Tambahan') {{ $memo->memotambahan->memo->kendaraan_id }}@else{{ $memo->kendaraan_id }} @endif"
                                        data-no_kabin="@if ($memo->kategori == 'Memo Tambahan') {{ $memo->memotambahan->memo->no_kabin }}@else{{ $memo->no_kabin }} @endif"
                                        data-no_pol="@if ($memo->kategori == 'Memo Tambahan') {{ $memo->memotambahan->memo->kendaraan->no_pol }}@else{{ $memo->kendaraan->no_pol }} @endif"
                                        data-param="{{ $loop->index }}">
                                        <td class="text-center">{{ $loop->iteration }}</td>
                                        <td>{{ $memo->kode_memo }}</td>
                                        <td>
                                            @if ($memo->kategori == 'Memo Tambahan')
                                                {{ $memo->memotambahan->memo->nama_driver }}
                                            @else
                                                {{ $memo->nama_driver }}
                                            @endif
                                        </td>
                                        <td>
                                            @if ($memo->kategori == 'Memo Tambahan')
                                                {{ $memo->memotambahan->memo->nama_rute }}
                                            @else
                                                {{ $memo->nama_rute }}
                                            @endif
                                        </td>
    ...
    <script>
        ...
        function getTarifs(Tarif_id, Kodetarif, NamaTarif, Nominal) {
            ...
            document.getElementById('harga_tarif').value = Nominal;
            // Close the modal (if needed)
        ...
        function updateHarga() {
            ...
            var hargasatuan = parseFloat($(".harga_tarif").val()) || 0;
            var jumlah = parseFloat($(".jumlah").val()) || 0;
            var biaya_tambahan = parseFloat($("#biaya_tambahan").val()) || 0;

            var harga = hargasatuan * jumlah;

            $(".total_tarif").val(harga);
            $(".total_tarif2").val(harga);
