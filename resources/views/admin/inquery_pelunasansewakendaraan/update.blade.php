...
@section('title', 'Inquery Faktur Pelunasan Sewa Kendaraan')
...
@section('content')
    ...
    <div class="content-header">
        <div class="containers-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Inquery Faktur Pelunasan Sewa Kendaraan</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
    ...
    <section class="content">
        <div class="containers-fluid">
            ...
            @endif
            <form action="{{ url('admin/inquery_pelunasansewakendaraan/' . $inquery->id) }}" method="POST"
                enctype="multipart/form-data" autocomplete="off">
                @csrf
                @method('put')
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Perbarui Faktur Pelunasan Sewa Kendaraan</h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                        <label style="font-size:14px" class="form-label" for="kode_tagihan">Kode invoice</label>
                        <div class="form-group d-flex">
                            <input class="form-control" hidden id="sewa_kendaraan_id" name="sewa_kendaraan_id"
                                type="text" placeholder=""
                                value="{{ old('sewa_kendaraan_id', $inquery->sewa_kendaraan_id) }}" readonly
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="form-group" hidden>
                                    <label for="vendor_id">return Id</label>
                                    <input type="text" class="form-control" id="vendor_id" readonly
                                        name="vendor_id" placeholder=""
                                        value="{{ old('vendor_id', $inquery->vendor_id) }}">
                                </div>
                                ...
                                <div class="form-group">
                                    <label style="font-size:14px" for="nama_vendor">Nama Rekanan</label>
                                    <input style="font-size:14px" type="text" class="form-control" id="nama_vendor"
                                        readonly name="nama_vendor" placeholder=""
                                        value="{{ old('nama_vendor', $inquery->nama_vendor) }}">
                                </div>
                                <div class="form-group" hidden>
                                    <div class="form-group">
                                        <label style="font-size:14px" for="alamat_vendor">Alamat
                                            return</label>
        ...
        <div class="modal fade" id="tableReturn" data-backdrop="static">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">Data Invoice Sewa</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <button type="button" class="btn btn-primary btn-sm"
                                                ...
                                                '{{ $return->detail_invoice->where('sewa_kendaraan.status_pelunasan', null)->pluck('sewa_kendaraan_id')->implode(', ') }}',
                                                '{{ $return->detail_invoice->where('sewa_kendaraan.status_pelunasan', null)->pluck('kode_faktur')->implode(', ') }}',
                                                '{{ $return->detail_invoice->where('sewa_kendaraan.status_pelunasan', null)->pluck('tanggal_memo')->implode(', ') }}',
                                                '{{ $return->detail_invoice->where('sewa_kendaraan.status_pelunasan', null)->pluck('sewa_kendaraan.grand_total')->implode(', ') }}',
                                                '{{ $return->detail_invoice->where('sewa_kendaraan.status_pelunasan', null)->pluck('total_faktur')->implode(', ') }}'
                                                )">
                                                <i class="fas fa-plus"></i>
                                            </button>
    ...
    <script>
        ...
        function removeBan(identifier, detailId) {
            ...
            $.ajax({
                url: "{{ url('admin/inquery_pelunasansewakendaraan/deletedetailpelunasanreturn') }}/" + detailId,
                type: "POST",
                data: {
                    _method: 'DELETE',
                    _token: '{{ csrf_token() }}'
    ...
    <script>
        ...
        function removeTambahan(identifier, detailId) {
            ...
            $.ajax({
                url: "{{ url('admin/inquery_pelunasansewakendaraan/deletedetailpelunasanpotongan') }}/" + detailId,
                type: "POST",
                data: {
                    _method: 'DELETE',
                    _token: '{{ csrf_token() }}'
    ...
    <script>
        $(document).ready(function() {
            $('#vendor_id').on('input', function() {
                ...
                if (pelangganID) {
                    $.ajax({
                        url: "{{ url('admin/pelunasan_sewakendaraan/get_fakturpelunasansewa') }}" + '/' +
                            pelangganID,
                        type: "GET",
                        dataType: "json",
    ...
    <script>
        ...
        function removeFaktur(identifier, detailId) {
            ...
            $.ajax({
                url: "{{ url('admin/inquery_pelunasansewakendaraan/deletedetailpelunasan') }}/" + detailId,
                type: "POST",
                data: {
                    _method: 'DELETE',
                    _token: '{{ csrf_token() }}'
        ...
        function itemPembelianfak(identifier, key, value = null) {
            ...
            item_pembelian += '<td hidden>';
            item_pembelian += '<div class="form-group">'
            item_pembelian += '<input type="text" class="form-control" id="sewa_kendaraan_id-' + key +
                '" name="sewa_kendaraan_id[]" value="' + sewa_kendaraan_id + '" ';
            item_pembelian += '</div>';
            item_pembelian += '</td>';
