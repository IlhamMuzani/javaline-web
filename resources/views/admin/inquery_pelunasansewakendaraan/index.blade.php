...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Inquery Faktur Pelunasan Pelunasan Sewa Kendaraan</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item active">Inquery Faktur Pelunasan Pelunasan Sewa Kendaraan</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
        ...
        <div class="container-fluid">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Data Inquery Faktur Pelunasan Pelunasan Sewa Kendaraan</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                                            ...
                                            @if ($fakturpelunasan->status == 'unpost')
                                                ...
                                                @if (auth()->check() && auth()->user()->fitur['inquery pelunasan ekspedisi update'])
                                                    <a class="dropdown-item"
                                                        href="{{ url('admin/inquery_pelunasansewakendaraan/' . $fakturpelunasan->id . '/edit') }}">Update</a>
                                                @endif
                                                @if (auth()->check() && auth()->user()->fitur['inquery pelunasan ekspedisi show'])
                                                    <a class="dropdown-item"
                                                        href="{{ url('admin/inquery_pelunasansewakendaraan/' . $fakturpelunasan->id) }}">Show</a>
                                                @endif
                                                @if (auth()->check() && auth()->user()->fitur['inquery pelunasan ekspedisi delete'])
                                                    <form style="margin-top:5px" method="GET"
                                                        action="{{ route('hapuspelunasansewa', ['id' => $fakturpelunasan->id]) }}">
                                                        <button type="submit"
    ...
    <script>
        ...
        function cari() {
            form.action = "{{ url('admin/inquery_pelunasansewakendaraan') }}";
            form.submit();
        }
    </script>
    ...
    {{-- <script>
        document.getElementById('editMemoekspedisi').addEventListener('dblclick', function() {
            window.location.href = "{{ url('admin/inquery_pelunasansewakendaraan/' . $fakturpelunasan->id . '/edit') }}";
        });
    </script> --}}
    ...
    <script>
        $(document).ready(function() {
            $('#hapus-null-button').click(function() {
                $.ajax({
                    url: "{{ url('admin/inquery_pelunasansewakendaraan/update_deleted_atpelunasan/') }}",
                    type: 'GET',
                    data: {
    ...
    <script>
        $(document).ready(function() {
            $('.unpost-btn').click(function() {
                ...
                $.ajax({
                    url: "{{ url('admin/inquery_pelunasansewakendaraan/unpost/') }}/" +
                        memoId,
                    type: 'GET',
                    data: {
                        id: memoId
    ...
    <script>
        $(document).ready(function() {
            $('.posting-btn').click(function() {
                ...
                $.ajax({
                    url: "{{ url('admin/inquery_pelunasansewakendaraan/posting/') }}/" +
                        memoId,
                    type: 'GET',
                    data: {
                        id: memoId
