...
<body>
    ...
    <table>
        ...
        @foreach ($inquery as $penggantianoli)
            <tr>
                <td class="kepala">{{ $penggantianoli->kode_penggantian }}</td>
                ...
                <td colspan="5">
                    <table>
                        ...
                        @foreach ($penggantianoli->detail_penggantianbearing as $detail_penggantian)
                            <tr>
                                <td class="detail">{{ $detail_penggantian->sparepart->kode_partdetail }}</td>
                                <td class="detail">{{ $detail_penggantian->sparepart->nama_barang }}</td>
                                <td class="detail">{{ $detail_penggantian->jumlah }}</td>
                                <td class="detail">{{ $detail_penggantian->km_penggantian }}</td>
                                <td class="detail">{{ $detail_penggantian->km_berikutnya }}</td>
                            </tr>
                        @endforeach
                    </table>
                </td>
