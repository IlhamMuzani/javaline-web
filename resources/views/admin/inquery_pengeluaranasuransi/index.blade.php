...
@section('title', 'Inquery Pengambilan Uang Asuransi Barang')
...
@section('content')
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item active">Inquery Pengambilan Uang Asuransi Barang</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
        ...
        <div class="container-fluid">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Data Inquery Pengambilan Uang Asuransi Barang</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                                        ...
                                        <td class="text-center">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                @if ($pengeluaran->status == 'unpost')
                                                    @if ($saldoTerakhir->sisa_asuransi < $pengeluaran->grand_total)
                                                    ...
                                                    @if (auth()->check() && auth()->user()->fitur['inquery pengambilan kas kecil update'])
                                                        <a class="dropdown-item"
                                                            href="{{ url('admin/inquery_pengeluaranasuransi/' . $pengeluaran->id . '/edit') }}">Update</a>
                                                    @endif
                                                    @if (auth()->check() && auth()->user()->fitur['inquery pengambilan kas kecil show'])
                                                        <a class="dropdown-item"
                                                            href="{{ url('admin/inquery_pengeluaranasuransi/' . $pengeluaran->id) }}">Show</a>
                                                    @endif
                                                    @if (auth()->check() && auth()->user()->fitur['inquery pengambilan kas kecil delete'])
                                                        <form style="margin-top:5px" method="GET"
                                                            action="{{ route('hapuspengeluaranasuransi', ['id' => $pengeluaran->id]) }}">
                                                            <button type="submit"
    ...
    <script>
        $(document).ready(function() {
            $('.unpost-btn').click(function() {
                ...
                $.ajax({
                    url: "{{ url('admin/inquery_pengeluaranasuransi/unpostpengeluaranasuransi/') }}/" +
                        memoId,
                    type: 'GET',
                    data: {
                        id: memoId
    ...
    <script>
        $(document).ready(function() {
            $('.posting-btn').click(function() {
                ...
                $.ajax({
                    url: "{{ url('admin/inquery_pengeluaranasuransi/postingpengeluaranasuransi/') }}/" +
                        memoId,
                    type: 'GET',
                    data: {
                        id: memoId
    ...
    <script>
        ...
        function cari() {
            form.action = "{{ url('admin/inquery_pengeluaranasuransi') }}";
            form.submit();
        }
    </script>
