...
@section('content')
                                            ...
                                            <td hidden>
                                                <div class="form-group">
                                                    <input type="text" class="form-control"
                                                        id="faktur_penjualanreturn_id-0"
                                                        name="faktur_penjualanreturn_id[]">
                                                </div>
                                            </td>
                                            <td>
                                                <div class="form-group">
        ...
        function GetReturn(Return_id, KodeReturn, Pelanggan_id, KodePelanggan, NamaPell, Telpel, AlamatPelanggan,
            ...
            for (var i = 0; i < Fakturekspedisi_id.length; i++) {
                ...
                formHtml += '<tr>' +
                    ...
                    '   </div>' +
                    '</td>' +
                    '<td style="font-size:14px">' + totalFormatted + '</td>' + // Display the formatted total
                    '</tr>';
    ...
    <script>
        ...
        function itemPembelian(urutan, key, value = null) {
            var faktur_penjualanreturn_id = '';
            var kode_potongan = '';
            var keterangan_potongan = '';
            var nominal_potongan = '';

            if (value !== null) {
                faktur_penjualanreturn_id = value.faktur_penjualanreturn_id;
            ...
            item_pembelian += '<input type="text" class="form-control" id="faktur_penjualanreturn_id-' + urutan +
                '" name="faktur_penjualanreturn_id[]" value="' + faktur_penjualanreturn_id + '" ';
            item_pembelian += '</div>';
            item_pembelian += '</td>';
    ...
    <script>
        ...
        function getPotongan(rowIndex) {
            ...
            $('#faktur_penjualanreturn_id-' + activeSpecificationIndex).val(Potongan_id);
    ...
    <script>
        var data_pembelian = @json(session('data_pembelianslain'));
        var jumlah_ban = 1;
    ...
    <script>
        function hitungSelisihx() {
            var totalPembayaranText = document.getElementById("totalPembayarans").value.replace(/\./g, "").replace(",",
                ".");
            var totalPembayaran = parseFloat(totalPembayaranText);
            console.log("Total Pembayaran: " + totalPembayaran);

            var nominal = parseFloat(document.getElementById("nominal").value.replace(/\./g, "").replace(",", "."));
            console.log("Nominal: " + nominal);

            var hasilSelisih = totalPembayaran - nominal;
            console.log("Hasil Selisih: " + hasilSelisih);

            var hasilselisihElement = document.getElementById("hasilselisih");
            if (hasilselisihElement) {
                if (!isNaN(hasilSelisih)) {
                    hasilselisihElement.innerText = hasilSelisih.toFixed(2);
                } else {
                    hasilselisihElement.innerText = "Hasil tidak valid";
                }
            } else {
                console.error("Elemen dengan id 'hasilselisih' tidak ditemukan.");
            }
        }

        document.getElementById("nominal").addEventListener("input", hitungSelisihx);
        // Panggil hitungSelisihx untuk menghitung hasil saat halaman dimuat
        hitungSelisihx();
    </script>
