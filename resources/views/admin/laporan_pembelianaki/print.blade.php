...
<body>
    <div class="container">
        <h1>FAKTUR PEMBELIAN AKI - RANGKUMAN</h1>
    </div>
    <div class="text">
        @php
    ...
    <table>
        ...
        @foreach ($inquery as $pembelian_aki)
            <tr>
                <td>{{ $pembelian_aki->kode_pembelianaki }}</td>
                <td> {{ $pembelian_aki->tanggal_awal }}</td>
                <td> {{ $pembelian_aki->supplier->nama_supp }}</td>
                <td> {{ $pembelian_aki->aki->count() }}</td>
                <td> Rp. {{ number_format($pembelian_aki->aki->sum('harga'), 0, ',', '.') }}</td>
            </tr>
        @endforeach
    </table>
    ...
    @foreach ($inquery as $pembelian_aki)
        @php
            $total += $pembelian_aki->aki->sum('harga');
        @endphp
    @endforeach
